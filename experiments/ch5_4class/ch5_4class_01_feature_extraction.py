...
Data:    SHAPE EEG epochs from categoriesbatch{1-4}/
         4 categories × 212 subjects (211 after excluding Subject 127)
         Each file: (1229, 34) — 1229 timepoints × 34 channels

Pipeline:
  1. Load raw EEG (1229 × 34)
  2. Downsample 4× → truncate to 256 timesteps
  3. Z-score per channel
  4. Run LIF reservoir (N=256, β=0.05, θ=0.5) on each channel
  5. Extract BSC6 (6 bins, t=10–70) → 256×6 = 1536 per channel
...
Runtime: ~20–40 minutes depending on hardware (844 obs × 34 channels each)
============================================================================
"""
import numpy as np
import os
...
# ═══════════════════════════════════════════════════════════════════════
DATA_DIR = './categories'  # parent directory containing categoriesbatch{1-4}/
...
def main():
    ...
    print(f"  Expected: {len(subjects_all)} subjects × 4 categories = {len(subjects_all)*4}")
    
    # ─── Initialize reservoir ─────────────────────────────────────────
    print(f"\nInitializing LIF reservoir: N={N_RES}, β={BETA}, θ={THRESHOLD}, seed={SEED}")
    ...
    print(f"  Reservoir: {N_RES} neurons × {TARGET_TIMESTEPS} timesteps per channel")
    ...
    print(f"{'='*70}")
    print(f"  {N_obs} observations × {N_ch} channels × {n_comp} PCA dimensions")
