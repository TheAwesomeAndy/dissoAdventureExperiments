...
Binary clinical detection using the dynamical metrics is near chance for all diagnoses ($49$--$56\%$), consistent with EXP-6.3's observation that the channel-averaged metrics are condition-sensitive but not clinically sensitive. In the layer ablation of Chapter~\ref{chap:arspi_net}, Section~\ref{sec:ch5_ablation}, the per-channel dynamical block yields the largest SUD balanced accuracy among the tested ARSPI-Net layers ($53.6\%$), but this near-chance value does not establish a clinically discriminative signal. The measured difference indicates that retaining spatial resolution changes the feature geometry and motivates further validation.
