...
This chapter pursues three interleaved scientific questions on the full $211$-subject SHAPE dataset. The first is a representation question: how much of the reservoir embedding space is occupied by condition-related signal versus subject-identity nuisance structure? The second is a graph-propagation question: under what finite-sensor graph conditions does message-passing propagation increase or decrease the detectability of a weak condition signal embedded in strong subject-level noise? The third is a clinical-topological question: when the same graph is analyzed descriptively rather than propagatively, what transdiagnostic network phenotypes emerge? The chapter then extends these questions through two additional analyses. A paired-granularity comparison repeats the pipeline at four-class resolution (Threat, Mutilation, Cute, Erotic), identifying the regime boundary where the reservoir's temporal coding advantage transitions from dominant to insufficient. A layer ablation experiment directly tests whether ARSPI-Net's discriminative embedding, dynamical trajectory descriptors (Chapter~\ref{chap:dynamics}), and graph-topological descriptors carry redundant or complementary information.

The experimental program produces five principal findings. First, variance decomposition reveals that subject identity accounts for $62.6\%$ of embedding variance, condition accounts for $8.7\%$, and residual within-subject variability accounts for $28.7\%$, a $7\times$ subject-to-condition dominance ratio. Removing the subject-identity component through per-subject centering increases balanced accuracy from $63.4\%$ to $79.4\%$, indicating that cross-subject evaluation obscures condition-discriminative structure in the reservoir representation. Second, a Propagation Operating Characteristic analysis demonstrates that no tested graph operator, including smoothing, high-pass, residual, difference-aware, and attention-based variants, exceeds the non-propagated baseline. Third, the same graph, treated as a \emph{descriptive topological object}, yields exploratory transdiagnostic network phenotypes, with the strongest nominal association observed for Substance Use Disorder across three complementary metrics ($p = 0.0004$ for the strongest parametric comparison). Fourth, the paired-granularity comparison identifies a regime boundary: the reservoir outperforms band-power by $+12.5$~pp at three-class but is outperformed by $-6.2$~pp at four-class, with the strongest nominal clinical association shifting from SUD to PTSD. Fifth, the layer ablation matrix demonstrates that the three response layers are operationally distinct: the embedding dominates emotion classification, while different clinical labels attain their largest measured effects in different layers.
...
This chapter contributes a signal-detection analysis, a propagation-regime characterization, a nuisance-structure analysis, a paired-granularity comparison, a layer-ablation analysis, and a centered baseline comparison. The clinical-label results are exploratory: the strongest SUD parametric comparison has $p = 0.0004$, but its label-permutation result is $p = 0.073$ and the larger-sample analysis is nested rather than an independent replication. The ablation results therefore establish operational differences among measured layers in this cohort, not validated disorder-specific explanations.
...
First, variance decomposition establishes that subject identity accounts for $62.6\%$ of embedding variance, with condition signal at $8.7\%$ ($7\times$ ratio). Subject-centering increases classification accuracy by $+13$ to $+21$~pp across all seven tested representations, including EEGNet ($72.0\% \to 89.1\%$), raw EEG ($70.5\% \to 88.4\%$), and ARSPI-Net ($59.4\% \to 78.8\%$). The cross-architecture pattern indicates that subject-covariance entanglement is not specific to the reservoir representation in this dataset.
...
Fourth, graph-topological analysis identifies nominal SUD emotional-reactivity associations across three complementary metrics ($p = 0.0004$, $0.003$, $0.012$). The strongest effect does not reach significance under the empirical label-permutation test ($p = 0.073$), and the nested-sample comparison is not an independent replication.
