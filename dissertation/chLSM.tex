...
The theoretical foundations of reservoir computing, reviewed in Chapter~\ref{chap:background}, give conditions under which a sufficiently rich reservoir can approximate time-invariant causal operators with fading memory \cite{Maass2002RealTime,boyd_chua_1985}. These are existence results, not guarantees for the finite reservoir or stimuli used here. The gap between a universality result and a functioning engineering system is filled by empirical characterization: systematic investigation of how membrane decay, firing threshold, network size, and feature extraction change the observed driven response.

The separation property motivates asking whether different inputs lead to distinguishable reservoir states \cite{Maass2002RealTime}. It does not follow from a single classification result, and a negative finite-time trajectory-divergence estimate does not by itself prove the Echo State Property or separation. The later dynamical analysis reports such estimates descriptively. This chapter makes the narrower measurements shown in its figures and limits its conclusions to the tested synthetic inputs.
...
The reservoir architecture and neuron model are described in Chapter~\ref{chap:background}, Sections~\ref{sec:neuron_models}--\ref{sec:discrete_lif}. Most archived figures in Modules~1--2 use 512 units with fixed random input and recurrent weights. The original figure-generation script was not retained, and the negative retained potentials in the low-threshold plots cannot be produced by the audited update, which floors the post-threshold retained membrane at zero. Modules~1--2 are therefore retained as legacy qualitative observations rather than verification of the current implementation. The corrected synthetic classification in Chapter~\ref{chap:embeddings} uses the included custom NumPy implementation and the update order stated in Section~\ref{sec:discrete_lif}.

\textbf{Reservoir size.} The archived figures in this chapter use 512 neurons. Chapter~\ref{chap:embeddings} separately examines a 256-neuron configuration. Results are not assumed to transfer unchanged across sizes; the later configuration is treated as a distinct empirical choice.
...
In the time domain, the isolated subthreshold LIF neuron has exponential impulse response $h(t) = (1/\tau_m)\,e^{-t/\tau_m}$ for $t \geq 0$. Lindeberg's time-causal scale-space construction provides an axiomatic context in which cascades of truncated exponentials arise as variation-diminishing causal smoothing kernels \cite{lindeberg_2016}. That result applies to the linear scale-space construction, not to the complete recurrent threshold-and-reset reservoir. Here it motivates, but does not uniquely determine, the membrane filter. The parameter $\tau_m$ controls the kernel width: larger $\tau_m$ integrates over a wider temporal interval, while smaller $\tau_m$ emphasizes recent input.
...
This has a direct connection to the general theory of reservoir memory. Dambre et al.\ show that, under their assumptions, the sum of orthogonal information-processing capacities is bounded by the number of accessible state variables \cite{dambre_2012}. Allocating capacity to nonlinear functions therefore leaves less capacity for delayed linear reconstruction, and conversely. That theorem does not prescribe a universally optimal $\tau_m$ for this spiking network. It instead motivates treating $\tau_m$ as a design variable that changes the empirical allocation between persistence and nonlinear transformation.
...
The subthreshold case gives an exact reference calculation. For a scalar unit
with $M[t]=\alpha M[t-1]+I[t]$, $\alpha=1-\beta$, suppose two inputs are equal
after time $t_0$. Then $|\Delta M[t_0+k]|=\alpha^k|\Delta M[t_0]|$.

\begin{proposition}[Effective subthreshold memory horizon]
\label{prop:subthreshold_horizon}
For tolerance $\varepsilon>0$ and initial difference
$0<|\Delta M[t_0]|\le M$, the influence is at most $\varepsilon$ whenever
\begin{equation}
k\ge K_\varepsilon
=\left\lceil\frac{\log(\varepsilon/M)}{\log\alpha}\right\rceil.
\end{equation}
For small $\beta$, $K_\varepsilon\approx \beta^{-1}\log(M/\varepsilon)$.
\end{proposition}

\begin{proof}
Repeated substitution gives $|\Delta M[t_0+k]|\le\alpha^kM$. Solving
$\alpha^kM\le\varepsilon$ for $k$ yields the bound. The approximation uses
$\log(1-\beta)=-\beta+O(\beta^2)$.
\end{proof}

This is exact for a fixed subthreshold itinerary. Spike events can erase,
preserve, or discontinuously change differences, so the horizon is a reference
timescale rather than a global theorem for the full reservoir.
...
The contraction language requires a boundary condition because the spike map is
discontinuous at threshold. Let $P_i[t]$ be the pre-threshold membrane and define
the threshold margin over a finite trajectory by
\begin{equation}
\gamma=\min_{i,t}|P_i[t]-M_{\mathrm{th}}|.
\end{equation}

\begin{lemma}[Spike-itinerary stability away from threshold]
\label{lem:spike_margin}
If $\gamma>0$ and a perturbed trajectory satisfies
$\max_{i,t}|P_i'[t]-P_i[t]|<\gamma$, then
$S_i'[t]=S_i[t]$ for every neuron and time in the analyzed window.
\end{lemma}

\begin{proof}
For each $(i,t)$, the perturbation is smaller than the distance from $P_i[t]$ to
the threshold. It therefore cannot move that value across the threshold, so the
threshold indicator is unchanged.
\end{proof}

Within such a margin-preserving region the system follows one affine/reset
itinerary and ordinary contraction calculations apply. At the boundary,
arbitrarily small perturbations may change a spike decision; global contraction
cannot be inferred merely from reset events. The empirical seed sweep below
therefore tests a property that the reset mechanism alone does not prove.
...
Lowering the threshold to $M_{\text{th}} = 0.5$ fundamentally altered the reservoir's operational dynamics, enabling robust spiking activity that was clearly and systematically modulated by input strength. The reproducibility conclusions in this section rest on the spike-count and firing-rate evidence; the archived summed-membrane traces noted below could not be regenerated from the retained source and are not treated as verified support for the reset dynamics.

The archived summed-membrane plot (Figure~\ref{fig:m2_summed_mempot_thresh0p5_actual}) contains sharp negative deflections at higher amplitudes. Those traces are inconsistent with the audited nonnegative retained-membrane update and cannot be regenerated from the retained source. They are preserved to document the archived analysis, but the negative excursions must not be interpreted as verified reset dynamics. The spike-count plots remain qualitative evidence that lowering the threshold changes firing in that archived realization.
...
\begin{figure}[H]
    ...
    \caption{Legacy summed-membrane traces for the low-threshold regime. The negative retained potentials are inconsistent with the audited nonnegative update, and the original generator was not retained; this panel is preserved as an archival record rather than validated numerical evidence.}
