...
Second, the subthreshold response of a single LIF unit is exponential. Lindeberg derives time-causal scale-space kernels under a particular set of axioms \cite{lindeberg_2016}, which provides mathematical context for exponential temporal smoothing. The recurrent, thresholded, and reset reservoir is not itself that linear scale-space system, so the scale-space result is motivation rather than a uniqueness theorem for the implemented network.

\subsection{Discrete-Time Implementation and the $\beta$--$\tau_m$ Relationship}
\label{sec:discrete_lif}
The experiments use a custom NumPy implementation of a discrete-time LIF-like reservoir; they do not call \texttt{snnTorch}'s \texttt{RLeaky} cell. Before thresholding, the membrane update for neuron $i$ takes the form
\begin{equation}
    M_i[t] = (1 - \beta)\, M_i[t-1]\,(1 - S_i[t-1]) + I_{\text{tot},i}[t]
    \label{eq:lif_discrete}
\end{equation}
where $\beta \in (0,1)$ is the leak fraction, $S_i[t-1] \in \{0,1\}$ is the previous spike indicator, and $I_{\text{tot},i}[t]$ is the total input current. The factor $(1-S_i[t-1])$ clears the retained membrane after a prior spike. The implementation then sets $S_i[t]=\mathbf{1}[M_i[t]\geq M_{\text{th}}]$, subtracts $S_i[t]M_{\text{th}}$, and clips the retained membrane below at zero. This order matters and is not identical to the default reset semantics of every LIF library.
...
\begin{equation}
    ...
    \label{eq:beta_tau}
\end{equation}
Equation~\eqref{eq:beta_tau} is exact only within that Euler discretization. Matching the exact exponential solution instead gives $1-\beta=e^{-\Delta t/\tau_m}$, or $\tau_m=-\Delta t/\ln(1-\beta)$. With $\beta=0.05$, these mappings give $20$ and approximately $19.5$ simulation steps, respectively. Any conversion to milliseconds must use the actual post-decimation step size; the parameter is not itself a measured biological membrane constant.

The implemented neuron also admits a useful boundedness guarantee. Write
$\alpha=1-\beta\in(0,1)$, let $\mathbf{R}[t]$ denote the retained membrane after
threshold subtraction and lower clipping, and assume
$\|\mathbf{u}[t]\|_\infty\le U$. With the induced infinity norm, define
\begin{equation}
B=\|\mathbf{W}^{in}\|_\infty U+\|\mathbf{W}^{rec}\|_\infty.
\end{equation}

\begin{theorem}[Bounded-input bounded-state property]
\label{thm:lif_bibs}
For the discrete update used in this dissertation,
\begin{equation}
\|\mathbf{R}[t]\|_\infty
\le \alpha^t\|\mathbf{R}[0]\|_\infty+
\frac{B(1-\alpha^t)}{1-\alpha}
\le \|\mathbf{R}[0]\|_\infty+\frac{B}{\beta}.
\end{equation}
Thus every bounded input sequence produces a bounded retained-membrane sequence,
regardless of the realized spike pattern.
\end{theorem}

\begin{proof}
Since $\|\mathbf{S}[t]\|_\infty\le1$, the input and recurrent spike currents
have norm at most $B$. Reset masking cannot increase the retained nonnegative
membrane, and threshold subtraction followed by clipping cannot make it exceed
the nonnegative pre-threshold value. Hence
$\|\mathbf{R}[t]\|_\infty\le \alpha\|\mathbf{R}[t-1]\|_\infty+B$.
Iteration gives the geometric sum above.
\end{proof}

Boundedness is weaker than the echo state property: it prevents divergence but
does not prove that two initial conditions converge through threshold changes.
That distinction is retained in Chapter~\ref{chap:dynamics}.
...
The apparent tension between the DPI and improved linear classification is
resolved by an elementary construction.

\begin{proposition}[Geometry can improve without information creation]
\label{prop:xor_geometry}
Let $X_1,X_2$ be independent and uniform on $\{-1,1\}$ and let
$Y=X_1X_2$. No affine classifier in the original coordinates
$X=(X_1,X_2)$ separates $Y$ perfectly. The deterministic feature
$\varphi(X)=X_1X_2$ is perfectly linearly separable and satisfies
$I(Y;\varphi(X))=I(Y;X)=H(Y)$.
\end{proposition}

\begin{proof}
The positive class consists of $(1,1)$ and $(-1,-1)$ and the negative class of
$(1,-1)$ and $(-1,1)$. Their convex hulls intersect at the origin, so no affine
hyperplane strictly separates them. In the transformed coordinate,
$\varphi(X)=Y$, and the threshold at zero separates the classes. Because $Y$
is a deterministic function of both $X$ and $\varphi(X)$, both mutual
informations equal $H(Y)$.
\end{proof}

This proposition formalizes the role claimed for the reservoir: it may rearrange
existing information into coordinates accessible to a restricted readout without
increasing the information available about the label.
...
The dynamical characterization framework developed in Chapter~6 relies on concepts from nonlinear dynamical systems theory. This subsection introduces the key constructs and, critically, the distinction between autonomous and driven dynamics that underpins the dissertation's measurement-instrument framing.
...
%% ===================================================================
\section{Reservoir Computing: A Framework for Computing with Complex Dynamics}
...
\begin{itemize}
    ...
    \item \textbf{Dynamical characterization stage}: Seven named descriptors (total spikes, mean firing rate, rate entropy, rate variance, temporal sparsity, permutation entropy, and autocorrelation decay) each has a physical unit and a direct interpretation in terms of reservoir response properties (excitability, temporal complexity, persistence).
...
\midrule
L1: Temporal traceability & $|\text{Corr}(b^*, \phi_k)| \geq 0.5$ & Pearson $r$ vs.\ P300 and LPP amplitude (pre- and post-reservoir) & Pre-reservoir: $r = 0.65$ (P300), $|r| = 0.82$ (LPP), per-channel peak $r = 0.837$. Post-reservoir (BSC$_6$ spike bins): $r = 0.33$ (P300), $|r| = 0.23$ (LPP). \\
\midrule
...
\midrule
L3: Dynamical characterization & $|\text{Corr}(\phi_k, \mathbf{d})| \geq 0.5$ per channel & Per-channel univariate correlation: amplitude descriptor vs.\ ERP scalar & Amplitude-domain: mean amplitude $|r| = 0.820$ (LPP), $r = 0.647$ (P300); strongest per-channel $r = 0.837$. Spike-domain (post-reservoir BSC$_6$): $|r| = 0.23$ (LPP), $r = 0.33$ (P300). \\
\midrule
L4: Systems-level coupling & $\kappa > 0$, $p < 0.001$ (permutation) & Wilcoxon signed-rank + subject-level confirmation & median $\kappa = 0.273$, $p = 4.98 \times 10^{-113}$ (844 obs); confirmed at $N = 211$ ($p < 0.001$) \\
\bottomrule
\end{tabular}
\end{table}

The four-level taxonomy is used here to organize the analyses performed in this dissertation; it is not a standardized benchmark and cannot support a categorical priority claim. ProtoPMed-EEG \cite{barnett_protopmed_2024}, NeuCube \cite{kasabov_neucube_2014}, and EEGNet with feature attribution answer different questions on different datasets. ARSPI-Net's contribution is the explicit calculation of quantities at all four proposed levels within one pipeline, while the reliability and clinical validity of those quantities remain separate empirical questions.
...
Liquid-state-machine emotion recognition from EEG predates this dissertation. Al Zoubi, Awad, and Kasabov used an LSM on DEAP to predict valence, arousal, and liking at multiple observation durations \cite{alzoubi_lsm_eeg_2018}. The novelty claim here is therefore not the first use of an LSM for EEG emotion recognition. The present work instead tests a fixed, fully specified LIF transformation on the larger transdiagnostic SHAPE cohort, retains electrode-wise intermediate quantities, compares coarse and fine affective label structures, and subjects temporal-bin and electrode organization to explicit destruction controls. Earlier ARSPI-Net conference papers introduced the framework on MNIST and a preliminary affective-EEG feature-extraction task \cite{lane_arspinet_2023,lane_arspi_2024}; this dissertation extends those prototypes with controlled reservoir characterization, a leakage audit, corrected raw-data reruns, dynamical summaries, and cross-summary analyses.
...
Recent spiking EEG systems also narrow the space in which novelty can be claimed. Fractal-SNN uses a trained multiscale spiking architecture \cite{li_fractal_snn_2023}; EESCN combines spatial, spectral, and temporal processing for emotion recognition \cite{xu_eescn_2024}; an adaptive graph-convolutional SNN with an LSTM models electrode relationships and temporal dependence \cite{gong_adaptive_graph_snn_2023}; and SpikeWavformer integrates wavelet analysis with spiking attention \cite{yuan_spikewavformer_2025}. These systems target predictive performance and learned feature extraction. ARSPI-Net's narrower engineering distinction is a fixed recurrent operator whose membrane states, spikes, temporal counts, and downstream summaries remain available for direct inspection. That distinction does not imply superior accuracy, physiological equivalence, or lower hardware energy without a hardware implementation.
...
Recent primary work also shows that reservoir design is not limited to unstructured random connectivity. Yang et al.\ initialized a modular Echo State Network and reported gains over a regular ESN on DEAP arousal, valence, and stress/calm tasks while using a smaller reservoir \cite{yang_modular_esn_2024}. That result strengthens the case for examining reservoir topology as a design variable, but it does not validate the fixed LIF reservoir used here: the neuron model, dataset, preprocessing, label construction, and validation target differ. The relevant lesson for this dissertation is methodological. A reservoir should be treated as a family of finite designs whose behavior must be measured under the target data regime, not as a generic consequence of universality theory.

\subsection{Validation Design, Leakage, and the Unit of Generalization}
The contemporary EEG literature makes evaluation design part of the scientific claim rather than a secondary implementation detail. Empirical audits show that participant overlap and test-informed model development can substantially inflate EEG results \cite{lee_eeg_leakage_2023,brookshire_eeg_leakage_2024,kinahan_eeg_reproducibility_2024}. The result is directly relevant whenever one participant contributes multiple epochs, conditions, or augmented windows: the split must occur at the participant level if the intended claim concerns unseen participants.

Participant-level splitting is necessary but not sufficient. Every data-adaptive step (standardization, feature selection, PCA, graph construction that uses cohort statistics, and hyperparameter selection) must be fitted using training data only and then applied unchanged to the held-out fold. Otherwise information about the evaluation cohort influences the representation before prediction. Repeated measurements also determine the estimand. A transformation that uses all conditions from a held-out participant, such as the panel centering analyzed later in this dissertation, may be valid for a repeated-measures panel question but is transductive and unavailable for single-record prospective use. These distinctions motivate the audit disclosures in Chapters~5--8 and the corrected fold-local implementation retained with the source package.

Finally, clinical association and clinical prediction are different evidentiary targets. Nominal group differences, selected channels, and the largest result among many diagnoses or feature families are exploratory unless the family of tests is prespecified and multiplicity is controlled. A classifier evaluated on one cohort is an internal-validation result, not a diagnostic instrument. External cohorts, calibration, decision-relevant endpoints, and prospective assessment are required before terms such as biomarker, diagnostic utility, or clinical deployment are justified. This dissertation therefore uses clinical labels to explore cohort structure and explicitly avoids claims of patient-level validity.

\subsection{The Emergence of Hybrid Neuromorphic-GNN Architectures}
The integration of neuromorphic principles with other deep learning architectures is an active and growing field. A particularly promising direction is the combination of SNNs and Graph Neural Networks (GNNs). GNNs are designed to perform computation on graph-structured data, making them ideal for modeling the network properties of multi-channel sensors like EEG. Recent work has begun to explore this combination. For instance, the Dy-SIGN model demonstrated a method for propagating spike-based information through a GNN for dynamic link prediction tasks, showcasing the potential for event-driven graph computation \cite{Yoon2024DySIGN}. Other research has explored using GNNs to analyze functional connectivity graphs derived from neural activity. Preliminary work by the present author established the initial ARSPI-Net architectural concept and demonstrated feasibility on a reduced feature set \cite{lane_arspi_2024}.
...
These comparisons do not claim that the alternatives are fundamentally incapable. The argument is narrower: for the specific combination of requirements in this dissertation (temporal expressivity, multichannel relational modeling, interpretable internal dynamics, and robustness under nonstationary input) the fixed-reservoir plus graph-readout architecture provides the most defensible engineering compromise.

\subsection{Interpretability Landscape: Positioning ARSPI-Net}
\label{sec:interp_landscape}

Table~\ref{tab:interp_comparison} maps four representative approaches onto the dissertation's proposed four-level taxonomy. The mapping is an analytic comparison, not an exhaustive systematic review or an externally validated ranking. ARSPI-Net was designed to calculate quantities in all four proposed categories within one pipeline; whether users find those quantities understandable or clinically useful requires separate human-factors and external-validation studies.
