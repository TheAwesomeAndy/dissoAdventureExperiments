...
This chapter compares two families of derived summaries: per-channel reservoir dynamics (Chapter~\ref{chap:dynamics}) and graph topology (Chapter~\ref{chap:arspi_net}). Their channel-wise association is called coupling. Because both families are transformations of the same trial-averaged EEG epoch, association does not establish a common latent neural mechanism or independence from shared preprocessing.

The preceding chapters established two distinct characterization axes for affective EEG processing in ARSPI-Net. Chapter~\ref{chap:dynamics} developed the dynamical axis, extracting per-electrode temporal descriptors from the reservoir's internal state evolution and demonstrating that these descriptors are condition-sensitive (all seven metrics distinguish emotional from non-emotional input) but not strongly clinically sensitive at the channel-averaged level. Chapter~\ref{chap:arspi_net} developed the spatial axis by treating electrodes as nodes in a functional connectivity graph, characterizing propagation regimes and exploratory topological clinical associations. The strongest nominal SUD comparison has parametric $p = 0.0004$, but its empirical label-permutation test yields $p = 0.073$. Each axis accesses different information: the temporal axis primarily encodes stimulus condition, while the spatial axis contains stronger between-subject variation.

The present chapter asks whether these two axes are aligned. In the fMRI structure--function coupling literature, alignment between local functional properties and global topological role is a marker of organized neural processing \cite{Deco2015,honey_network_2007,suarez_linking_2020}. If an electrode that occupies a central position in the functional graph also produces a distinctive reservoir trajectory, then the temporal and spatial characterizations are coupled views of the measured signal organization rather than independent descriptions. If they are not aligned, the two axes access different layers of the EEG representation. The coupling measure $\kappa$ quantifies whether temporal dynamics and spatial topology covary across electrodes; its interpretation is organizational and does not, by itself, establish clinical utility.

The chapter reports archived four-category analyses (Threat, Mutilation, Cute, Erotic) and a separate three-condition analysis (Negative, Neutral, Pleasant). These are not replications: the input grouping, preprocessing, and archived reservoir implementations differ. In particular, the original four-category script omitted the post-spike threshold subtraction and nonnegative floor used by the three-condition pipeline. The audited script now uses the common update, but the raw four-category EEG was unavailable for regeneration. Cross-analysis numerical differences are therefore descriptive and cannot be attributed to granularity alone.
...
\begin{proposition}[Range of the coupling statistic]
\label{prop:kappa_range}
For any $p\times q$ matrix of correlation coefficients,
$0\le\kappa\le1$. Equality $\kappa=1$ requires
$|C_{jk}|=1$ for all $j,k$.
\end{proposition}

\begin{proof}
Each correlation satisfies $C_{jk}^2\le1$, so
$0\le\sum_{j,k}C_{jk}^2\le pq$. Taking the square root and dividing by
$\sqrt{pq}$ gives the bound. The upper equality requires equality in every
summand.
\end{proof}
...
The electrode-permutation null shuffles the row indices of $\mathbf{T}_{s,c}$ relative to $\mathbf{D}_{s,c}$, breaking the electrode correspondence while preserving the marginal distributions of both descriptor families. For each of 5,000 permutations~$\pi$, a null coupling $\kappa_{s,c}^{(\pi)}$ is computed. The per-observation $p$-value is
\begin{equation}
p_{s,c} = \frac{1 + \#\{\kappa_{s,c}^{(\pi)} \ge \kappa_{s,c}\}}{1 + 5000}.
\end{equation}

The null has a finite-array baseline that can be derived exactly. Consider any
two nonconstant rank vectors across $C$ electrodes. Center and scale them to unit
Euclidean norm, calling them $a$ and $b$, so their Spearman correlation after a
random row permutation is $r_\pi=a^\top P_\pi b$.

\begin{theorem}[Second moment of permutation coupling]
\label{thm:permutation_coupling}
Under a uniformly random permutation of one electrode vector,
\begin{equation}
\mathbb{E}_\pi[r_\pi]=0,
\qquad
\mathbb{E}_\pi[r_\pi^2]=\frac{1}{C-1}.
\end{equation}
Consequently, for the normalized Frobenius statistic in
Equation~\eqref{eq:kappa},
\begin{equation}
\mathbb{E}_\pi[\kappa_\pi^2]=\frac{1}{C-1},
\end{equation}
even when the $pq$ correlation entries are mutually dependent.
\end{theorem}

\begin{proof}
Centering gives $\sum_i a_i=\sum_i b_i=0$, and unit scaling gives
$\sum_i a_i^2=\sum_i b_i^2=1$. For a uniform permutation,
$\mathbb{E}[b_{\pi(i)}]=0$, proving the first identity. Further,
$\mathbb{E}[b_{\pi(i)}^2]=1/C$ and, for $i\ne j$,
$\mathbb{E}[b_{\pi(i)}b_{\pi(j)}]=-1/[C(C-1)]$. Therefore
\begin{align}
\mathbb{E}[r_\pi^2]
&=\frac{1}{C}\sum_i a_i^2
-\frac{1}{C(C-1)}\sum_{i\ne j}a_i a_j\\
&=\frac{1}{C}+\frac{1}{C(C-1)}=\frac{1}{C-1},
\end{align}
because $\sum_{i\ne j}a_i a_j=-\sum_i a_i^2=-1$. Finally,
$\kappa^2=(pq)^{-1}\sum_{j,k}r_{jk}^2$; linearity of expectation gives the
last result without requiring independence across entries.
\end{proof}

For $C=34$, the null root-mean-square correlation and root-mean-square coupling
are both $1/\sqrt{33}\approx0.174$. This is not the mean of $\kappa$; Jensen's
inequality gives only $\mathbb{E}[\kappa]\le1/\sqrt{33}$. The result explains why
a positive raw $\kappa$ is expected even under broken electrode correspondence
and why inference must compare against the permutation distribution rather than
zero.
...
The archived row-level Wilcoxon test on $\kappa_{\text{obs}}-\kappa_{\text{null median}}$ yields $W=338{,}144$ and $p=4.98\times10^{-113}$; the mean difference divided by its row-level standard deviation is 1.063. Repeated category rows make the $p$-value anti-conservative, and the standardized difference is not an independent-subject effect size. The descriptive shift applies only to the specified electrode-permutation null.
...
One possible explanation is that channel indices with concentrated theta-band phase relationships also have more regular temporal structure, which the reservoir summaries reflect. However, the within-epoch phase statistic, signal amplitude and spectrum, common reference, filtering, and the reservoir transform are all potential shared causes. The analysis did not measure ``input diversity'' or isolate any of these mechanisms. Because the supplied files lack montage labels and coordinates, no anatomical localization is claimed.

The data-processing inequality does not identify the cause or direction of these correlations \cite{cover_elements_2006}; it only constrains information under deterministic transformation.
...
The mean coupling scalar at three-class is $\kappa = 0.221 \pm 0.111$. None of the $5{,}000$ permutation-null realizations exceeds the observed value, so the finite-resolution empirical result is reported as $p < 2.0 \times 10^{-4}$ rather than $p = 0$. Coupling exceeds the tested permutation null at both granularities (Figure~\ref{fig:ch7_3class_coupling}).

The coupling matrix at three-class reveals a more mixed pattern than the uniformly negative four-class result. Of the $14$ entries, $6$ are negative and $8$ are positive. Rate variance ($+0.100$ with strength), temporal sparsity ($+0.107$), and $\tau_{AC}$ ($+0.088$) are positively coupled with connectivity: high-connectivity channels produce more variable, sparser, and more persistent reservoir dynamics. Total spikes ($-0.054$) and MFR ($-0.054$) are negatively coupled, indicating that high-connectivity channels fire fewer but more temporally structured spikes. These associations characterize the measured relationship between phase connectivity and reservoir response; they do not establish a causal effect of phase locking on reservoir dynamics.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/chSynthesis/fig_3class_coupling_existence.pdf}
    \caption{Coupling existence at three-class resolution ($N = 211$, $633$ observations). Left: group-mean $7 \times 2$ coupling matrix showing mixed positive/negative pattern ($6/14$ negative, $8/14$ positive), contrasting with the uniformly negative four-class result. Right: permutation test ($5{,}000$ shuffles). Observed $\kappa = 0.221$ exceeds every null realization ($p < 2.0 \times 10^{-4}$ at $5{,}000$ permutations).}
...
Combined T+D features ($306$ dimensions) do not consistently outperform T-only ($68$-D) or D-only ($238$-D) on any clinical detection task at three-class (Figure~\ref{fig:ch7_3class_augmentation}). The four-class ADHD result (D-only AUC $= 0.622$, with $\kappa$ augmentation $p = 0.001$) does not replicate at three-class, where all accuracies are near chance ($43$--$53\%$). The ADHD result is therefore resolution-dependent and exploratory; the present data do not establish a general coupling-derived clinical signal.
...
Two coupling effects appear only at four-class resolution: the Cute--Erotic coupling difference ($p = 0.025$) concentrated in temporal-structure metrics and the ADHD $\kappa$-augmentation result ($p = 0.001$). Neither replicates at three-class. These resolution-dependent findings motivate prespecified testing in an independent cohort rather than a general claim of clinical coupling.

\subsection{The Three-Layer Architecture: An Integrated View}

The complete experimental program across Chapters~\ref{chap:arspi_net}--\ref{chap:synthesis} supports the following characterization of ARSPI-Net as an intrinsically interpretable measurement framework. The architecture reveals three operationally distinct response layers in affective EEG, each producing named, inspectable quantities whose prospective clinical use remains to be evaluated:
\begin{enumerate}
    ...
    \item The \textbf{topology/coupling layer} (graph-topological descriptors and electrode-wise coupling $\kappa$) contains the strongest nominal clinical associations at the graph level (SUD parametric $p = 0.0004$, permutation $p = 0.073$; GAD degree heterogeneity $p = 0.032$) and a within-valence coupling difference at four-class resolution. Coupling $\kappa$ quantifies whether temporal and spatial characterizations covary across electrodes.
\end{enumerate}
These layers are not interchangeable. In the layer ablation of Chapter~\ref{chap:arspi_net}, different clinical labels attain their largest measured effects in different layers: dynamics for SUD and PTSD, topology for GAD, and coupling for ADHD. Because several effects are uncorrected, resolution-dependent, or not permutation-significant, the result establishes layer-dependent measurement sensitivity in this cohort rather than validated disorder-specific explanations.
...
\end{itemize}
No individual chi-squared test for diagnosis$\times$prototype-class association reaches $p < 0.05$ (all $p > 0.07$, Cram\'{e}r's $V < 0.16$), consistent with the small per-prototype sample sizes ($n = 5$--$40$). The prototype profiles are therefore descriptive and hypothesis-generating. They do not establish diagnostically structured patient subgroups or clinically usable subtypes.

Level~4 interpretability is operationalized as systems-level information unavailable from temporal or spatial characterization alone. Condition assignment, prototype purity, transition structure, ERP profile, and coupling $\kappa$ form a multi-dimensional cohort characterization. Their reliability for patient-level interpretation is not tested. This systems-level correspondence completes the four-level measurement chain that begins with temporal traceability (Chapter~\ref{chap:embeddings}), passes through geometric transparency (Chapter~\ref{chap:arspi_net}) and dynamical characterization (Chapter~\ref{chap:dynamics}), and concludes here with cross-scale correspondence.

Within the systems compared in Chapter~\ref{chap:background}, ProtoPMed-EEG \cite{barnett_protopmed_2024} provides prototype matching but does not measure cross-scale coupling; NeuCube \cite{kasabov_neucube_2014} provides connectivity analysis but does not formalize coupling between temporal descriptors and spatial topology; and EEGNet with SHAP does not expose a comparable systems-level variable. The present contribution is the explicit definition and measurement of cross-scale coupling in a neuromorphic EEG pipeline. The clinical prototype patterns remain exploratory because no diagnosis association reaches $p < 0.05$.
