...
This chapter pursues three interleaved scientific questions on the SHAPE dataset. The first is a representation question: what condition information is retained by a fixed spiking reservoir after fold-safe compression? The second is a graph-propagation question: under what finite-sensor graph conditions does message-passing increase or decrease the detectability of a weak condition signal? The third is a clinical-topological question: what exploratory associations appear when the same graph is treated descriptively? A paired-granularity comparison evaluates three conditions (Negative, Neutral, Pleasant) and four affective categories (Threat, Mutilation, Cute, Erotic). A layer ablation asks whether the discriminative embedding, dynamical trajectory descriptors (Chapter~\ref{chap:dynamics}), and graph-topological descriptors carry redundant or complementary information. The post-defense audit separates corrected experiments from results retained from the defended workflow.

The corrected experiments produce four principal findings. First, the fixed reservoir BSC$_6$/SRP-64 representation reaches $60.6\%$ balanced accuracy for three conditions and $53.2\%$ for four categories, exceeding the matched conventional baseline by $11.3$ and $16.5$ percentage points. Participant-label permutation tests give $p=0.0099$ for both tasks. Second, destroying temporal-bin order, electrode identity, or both reduces performance, identifying temporal order and spatial identity as contributors rather than attributing the result to total spike count alone. Third, the result is stable across three prespecified reservoir seeds; early, middle, and late window ablations show that useful information is distributed over the epoch. Fourth, raw ERP summaries remain stronger ($68.1\%$ and $60.5\%$), so the contribution is a structured, inspectable transform rather than best-in-class accuracy. The archived graph and clinical analyses remain useful as exploratory records, but their global preprocessing and broad multiplicity prevent confirmatory inference.

The chapter's central contribution is a reproducible boundary around the evidence. The corrected analysis tests the fixed reservoir under two affective granularities and targeted destruction controls. The archived graph-propagation, centering, and layer-ablation results generate hypotheses about representation geometry and complementarity, but do not establish universal graph regimes, prospective centering gains, or diagnostic biomarkers.
...
The smoothing mechanism can be characterized independently of the learned
weights. Add self-loops, $\widetilde A=A+I$, let
$\widetilde D_{ii}=\sum_j\widetilde A_{ij}$, and define the symmetric propagation
operator
\begin{equation}
S=\widetilde D^{-1/2}\widetilde A\widetilde D^{-1/2}.
\end{equation}
Let $\Pi$ be the orthogonal projector onto the eigenspace of $S$ associated with
eigenvalue one; for a disconnected graph this space contains one stationary
degree-weighted vector per component.

\begin{theorem}[Component-wise graph smoothing]
\label{thm:graph_smoothing}
Assume an undirected nonnegative graph with self-loops. Let
$\rho=\max\{|\lambda|:\lambda\in\sigma(S),\lambda\ne1\}<1$. Then for every node
feature matrix $H$ and integer $k\ge0$,
\begin{equation}
\|S^kH-\Pi H\|_F\le \rho^k\|(I-\Pi)H\|_F.
\label{eq:graph_contraction}
\end{equation}
Thus repeated propagation exponentially removes all feature contrast orthogonal
to the component-wise stationary subspace.
\end{theorem}

\begin{proof}
Because $S$ is real symmetric, write $S=Q\Lambda Q^\top$ with orthonormal
eigenvectors. Split $Q$ into the eigenvalue-one subspace and its orthogonal
complement. On the first subspace $S^k$ is the identity; on the complement the
spectral norm of $S^k$ is at most $\rho^k$. Applying this operator to each feature
column and summing squared norms gives Equation~\eqref{eq:graph_contraction}.
\end{proof}

\begin{corollary}[Depth needed to erase a specified contrast]
If $0<\rho<1$, reducing the nonstationary contrast to a fraction $\varepsilon$
requires
$k\ge \lceil\log(\varepsilon)/\log(\rho)\rceil$ propagation steps.
\end{corollary}

The theorem predicts representation homogenization, not a universal decrease in
classification accuracy. Accuracy falls only when the removed subspace contains
task-relevant contrast that the learned transformations do not reconstruct. The
experiments below test precisely that additional empirical condition.
...
The raw-data audit identified a mismatch between this specification and the supplied legacy feature pickle. The pickle's $633\times256\times34$ array is exactly the result of taking every fourth row from rows $0$--$1023$ and z-scoring each epoch and channel. It therefore includes the $205$-sample prestimulus segment, omits the last $205$ poststimulus samples, and performs no anti-alias filtering. For three representative conditions from Subject~6, the maximum absolute difference between the pickle and that stride operation is zero, whereas correlations between the legacy and corrected arrays are $0.27$, $0.06$, and $0.30$. The corrected validation below is rebuilt from the text files and does not use the pickle for headline results. Subject~127 is excluded from both granularities because a recording is constant; Subject~195 is additionally excluded from the four-category analysis because the Cute recording contains a constant channel. The corrected sample sizes are therefore $211$ participants ($633$ observations) for three conditions and $210$ participants ($840$ observations) for four categories.
...
The stability matrix (center panel) reveals that the stable edges are not uniformly distributed: certain channel pairs form consistently strong connections across all resampled populations, while others are sensitive to which subjects are included. The core stable graph (right panel) contains the $109$ reproducible edges and defines the subset used for the most stable descriptive interpretation.
...
The chapter now distinguishes a corrected confirmatory stream from the archived exploratory stream. The corrected stream uses five repeats of five-fold participant-grouped validation. Every participant's complete condition panel remains in one fold. Per-feature standardization and the linear logistic readout are fitted on training participants only. Instead of PCA, BSC$_6$ is compressed from $1{,}536$ to $64$ coordinates with a fixed sparse random projection shared across electrodes. Because that projection is generated from a prespecified seed and does not estimate a basis from the observations, held-out participants cannot influence it. Reported intervals are participant-bootstrap $95\%$ intervals over the repeated out-of-fold predictions. A separate deterministic ridge readout is used for $100$ within-participant label permutations.

The corrected results are shown in Table~\ref{tab:corrected_validation} and Figure~\ref{fig:corrected_validation}. For three conditions, BSC$_6$/SRP-64 reaches $60.6\%$ balanced accuracy (bootstrap interval $57.8$--$63.4\%$); for four categories it reaches $53.2\%$ ($50.6$--$55.8\%$). The ridge permutation tests give $p=0.0099$ at both granularities. Conventional band-power plus Hjorth features reach $49.4\%$ and $36.7\%$, respectively. The fixed reservoir therefore exceeds the conventional baseline by $11.3$ percentage points for three conditions (bootstrap interval $7.9$--$14.7$) and $16.5$ points for four categories ($13.3$--$19.8$). Fusion adds $1.3$ points at three conditions ($0.0$--$2.6$) and no stable gain at four categories.

\begin{table}[t]
\centering
\caption{Corrected raw-data validation. Values are mean balanced accuracy across five repeats of five-fold participant-grouped validation; brackets are participant-bootstrap $95\%$ intervals.}
\label{tab:corrected_validation}
\small
\begin{tabular}{lcc}
\hline
\textbf{Representation} & \textbf{Three conditions} & \textbf{Four categories} \\
\hline
Raw ERP, 16 temporal bins & $68.1\%$ $[65.1,71.1]$ & $60.5\%$ $[57.9,63.0]$ \\
Conventional spectral/Hjorth & $49.4\%$ $[46.8,51.9]$ & $36.7\%$ $[34.6,38.9]$ \\
BSC$_6$/SRP-64 & $60.6\%$ $[57.8,63.4]$ & $53.2\%$ $[50.6,55.8]$ \\
BSC$_6$/SRP-64 + conventional & $62.0\%$ $[59.1,64.8]$ & $53.5\%$ $[50.8,56.3]$ \\
\hline
Chance & $33.3\%$ & $25.0\%$ \\
\hline
\end{tabular}
...
The destruction controls test what the reservoir representation uses. Independently shuffling the six temporal bins within each observation and electrode lowers accuracy by $7.4$ points at three conditions ($3.7$--$11.1$) and $12.9$ points at four categories ($9.9$--$16.0$). Collapsing time lowers accuracy by $4.9$ and $8.6$ points. Independently shuffling electrode order lowers accuracy by $13.9$ and $20.1$ points. Thus both temporal order and stable electrode identity contribute to held-out classification; the result does not require an anatomical interpretation of unlabeled channels.

Across reservoir seeds $7$, $42$, and $99$, three-condition accuracy ranges from $60.5\%$ to $62.1\%$. Temporal-window ablation gives $60.5\%$ for $39$--$273$~ms, $56.9\%$ for $273$--$508$~ms, $56.7\%$ for $508$--$742$~ms, $48.5\%$ for $742$--$977$~ms, and $63.3\%$ when six bins span $39$--$977$~ms. The full-epoch gain shows that later activity adds information, while the earliest window is the strongest isolated segment.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/chGraphNeuralNetworks/fig_corrected_novelty_validation.pdf}
    \caption{Corrected participant-held-out validation. A: primary baselines with participant-bootstrap $95\%$ intervals. B: destruction controls for temporal and electrode organization. C: temporal-window ablation; the vertical marker at the early window spans seeds 7, 42, and 99. Dotted lines show chance.}
    \label{fig:corrected_validation}
\end{figure}

\subsection{Baseline Methods}

The following PCA/SVM results are retained to document the defended workflow (Table~\ref{tab:baselines}). They were generated from the legacy stride-preprocessed pickle and a globally fitted PCA basis. They are not the corrected estimates and are not used for the chapter's primary inference.
...
\begin{table}[t]
\centering
\caption{Archived baseline classification ($N = 211$ subjects, $633$ observations). PCA was fitted globally and the input pickle did not implement the stated preprocessing; values are descriptive legacy estimates.}
\label{tab:baselines}
\begin{tabular}{llccc}
\hline
...
In that archived workflow, the concatenated embedding is $12.5$ points above band-power and $1.6$ points above pairwise relational features. Because the input and PCA differ from the corrected specification, those gaps cannot identify an architectural or sample-size mechanism. The corrected comparison in Table~\ref{tab:corrected_validation} supplies the defensible estimate.

\subsection{Message-Passing GNN Experiments}

Three architectures were evaluated in the archived workflow as five-seed ensembles with concatenated graph readouts classified by SVM (Figure~\ref{fig:gnn_comparison}, Table~\ref{tab:gnn_results}). The displayed concatenated baseline is higher than each message-passing value. The fold-wise Wilcoxon values are descriptive because cross-validation folds share training participants and do not constitute independent replicates; global preprocessing further prevents a corrected performance claim.
...
\begin{table}[t]
\centering
\caption{Archived message-passing results at $N = 211$. The non-propagated value is higher in this workflow, but the global preprocessing and dependent fold comparison do not establish a general regime boundary.}
...
At $N = 115$, the archived GIN and EdgeConv values are $51.6\%$ and $53.0\%$; at $N = 211$, they are $49.8\%$ and $48.5\%$, while the concatenated value changes from $60.6\%$ to $63.4\%$. The smaller cohort is nested in the larger cohort and the same global pipeline is reused, so this is not an independent replication or evidence of a fundamental sample-size law. It is a hypothesis that should be retested with verified electrode labels, fold-local graph construction, and an external cohort.


\subsection{Conventional End-to-End Baselines}

The preceding sections compare ARSPI-Net against spectral features and graph-propagation variants. Table~\ref{tab:conventional_baselines} retains the defended comparison with trainable models for completeness. These values use the archived input pickle, global preprocessing, and, in the centered columns, the complete held-out participant panel. They are descriptive and are not used as corrected prospective estimates.

\begin{table}[t]
\centering
\caption{Archived baseline comparison ($N = 211$, $633$ observations). Deep models use $5$-seed ensembles with majority voting. Global preprocessing and complete-panel centering make these descriptive legacy estimates, not prospective performance.}
...
\includegraphics[width=0.85\textwidth]{pictures/chGraphNeuralNetworks/fig_baseline_comparison.pdf}
    \caption{Archived baseline comparison on the three-condition SHAPE task. Left bars: uncentered. Right bars: complete-panel centered. The shared increase is descriptive; global preprocessing and the transductive centering operation prevent a prospective or architecture-independent claim. Chance $=33.3\%$.}
\label{fig:baseline_comparison}
\end{figure}

Within this archived workflow, EEGNet has the highest displayed uncentered value ($72.0\% \pm 4.9\%$), the reservoir value is similar to the GRU and LSTM values, and the raw-EEG linear readout exceeds the reservoir. These are observations about this implementation, not evidence that the gap is caused by a particular theoretical tradeoff.

Complete-panel centering increases every displayed value, but the operation uses all conditions for each held-out participant. It is therefore a transductive repeated-measures contrast, unavailable when a prospective participant contributes one recording. The centered values cannot establish that centering dominates architecture choice or that the resulting scores are deployable.

\paragraph{Statistical scope.} The original intervals summarize resampling variation inside the archived workflow. They do not repair global preprocessing or the transductive estimand. A prospective centering method would require calibration data available at prediction time and independent evaluation.
...
At $78.8\%$ centered accuracy, ARSPI-Net supports discriminative classification, seven dynamical descriptors (Chapter~\ref{chap:dynamics}), and graph-topological analysis (Sections~\ref{sec:ch5_interpretability}--\ref{sec:ch5_ablation}). Its fixed recurrent weights make the transformation reproducible for a specified initialization and parameter set. Some intermediate variables have physical units; others, including PCA loadings and graph summaries, are dimensionless. The advantage is explicit construction and inspectability, not demonstrated clinical meaning for every variable.

The archived accuracy gap is consistent with information loss during nonlinear transformation and BSC$_6$ compression, but it is not a direct measurement of the memory--nonlinearity tradeoff described by Dambre et al.\ \cite{Dambre2012InformationProcessing}. The corrected comparison likewise shows that raw ERP summaries outperform BSC$_6$/SRP-64. The reservoir's defensible advantage is the availability of a specified sequence of membrane, spike, temporal-bin, and graph-organized quantities for analysis.
...
\textbf{Bhattacharyya distance} summarizes class overlap under a Gaussian approximation and can participate in bounds on Bayes error; the distance itself is not an upper bound on the error rate. Here $D_B = \tfrac{1}{8}(\boldsymbol{\mu}_0 - \boldsymbol{\mu}_1)^\top \boldsymbol{\Sigma}^{-1}_{\text{avg}} (\boldsymbol{\mu}_0 - \boldsymbol{\mu}_1) + \tfrac{1}{2}\ln\tfrac{|\boldsymbol{\Sigma}_{\text{avg}}|}{|\boldsymbol{\Sigma}_0|^{1/2}|\boldsymbol{\Sigma}_1|^{1/2}}$, computed under a diagonal covariance approximation.
...
An earlier version labeled $\tfrac{1}{2}\sum_j\ln(1+d_j'^2)$ as a mutual-information lower bound. That expression is not a valid general bound for these correlated multiclass features and can exceed the entropy of the three-class label. It is therefore not interpreted as mutual information.
...
\begin{figure}[H]
    ...
    \caption{Archived class-geometry summaries for Negative versus Neutral. The panel labeled mutual information used an invalid estimator and is not interpreted. PCA-dependent quantities are descriptive because the archived workflow fitted PCA before cross-validation.}
    \label{fig:exp08_detection}
\end{figure}

The archived geometry summaries show larger d$'$, Bhattacharyya distance, FDR, and Mahalanobis distance after panel centering. The values are descriptive: the PCA-dependent pipeline fitted PCA globally, several candidate representations were compared on the same cohort, and no nested analysis treats the differences as confirmatory. The values previously reported as $23.7$ and $47.3$ nats are invalid as label mutual information because a three-class label has entropy no greater than $\ln 3$ nats.
...
Within the archived sweep, every tested adjacency produces a lower displayed value than no propagation. This pattern is consistent with smoothing of inter-channel contrast, but global preprocessing and the limited set of graph constructions prevent attributing the result generally to graph size or density.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pictures/chGraphNeuralNetworks/exp04_graph_construction_sweep.pdf}
    \caption{Archived graph construction sweep. Pearson, spatial, and random adjacencies produce lower displayed values at $K=2$ than at $K=0$ in this workflow; the result is hypothesis-generating rather than a general regime boundary.}
...
The preceding sections establish that graph propagation does not improve classification in this regime. A natural question follows: does the graph representation carry information that flat concatenation discards? The graph, treated as a \emph{descriptive topological object} rather than a propagative inference substrate, yields candidate transdiagnostic network phenotypes. The analyses in this section are explicitly exploratory: they survey multiple clinical variables, graph metrics, and interaction terms. The strongest nominal SUD effect is re-examined by label permutation and by a nested-sample stability check. The empirical permutation result does not reach $p < 0.05$, so no effect is presented as a validated biomarker. The contribution is the identification of candidate network phenotypes for targeted investigation in future confirmatory studies.
...
\textbf{SUD $\times$ Efficiency Reactivity ($p = 0.003$, $d = +0.43$; uncorrected).} The archived groups have opposite signed mean contrasts ($\Delta=+0.022$ and $-0.020$). This nominal interaction does not establish a distinct mode of network reorganization.
...
To evaluate sensitivity to random group assignment, a permutation test was conducted on the strongest nominal effect (SUD $\times$ Strength Reactivity). The observed Cohen's $d = -0.34$ is $2.3$ standard deviations from the mean of the null distribution generated by $5{,}000$ label permutations (Figure~\ref{fig:exp05_permutation}), but the empirical permutation test does not reach significance ($p = 0.073$). The parametric result therefore remains hypothesis-generating rather than permutation-validated.
...
This nested-sample stability check distinguishes effects that persist as the sample expands from $N = 115$ to $N = 211$ from effects that do not. Because the samples overlap, it is not an independent replication. The SUD interactions are retained as the more stable exploratory findings, while independent-cohort confirmation remains necessary.
...
\section{Corrected Paired-Granularity Comparison}
\label{sec:ch5_4class}

The corrected pipeline evaluates three conditions (Negative, Neutral, Pleasant) and four affective categories (Threat, Mutilation, Cute, Erotic). Subject~127 is excluded from both tasks for a constant recording; Subject~195 is additionally excluded from the four-category task for a constant Cute recording. The resulting samples contain $211$ participants and $633$ observations at three conditions, and $210$ participants and $840$ observations at four categories.

\subsection{Persistence of Temporal Coding Across Granularities}

Table~\ref{tab:regime_comparison} presents the corrected comparison. The reservoir representation exceeds the conventional band-power/Hjorth baseline at both granularities: $+11.3$ percentage points at three conditions and $+16.5$ points at four categories. Thus, the previously displayed reversal does not survive reconstruction from raw files. The archived reversal combined a nonmatching time slice, stride-based downsampling, global PCA, and unequal quality-control samples; it is not retained as a scientific finding.

\begin{table}[t]
\centering
\caption{Corrected cross-granularity comparison. Intervals are participant-bootstrap $95\%$ intervals around predictions pooled from five repeats of five-fold participant-grouped cross-validation.}
...
Conventional + reservoir fusion & $62.0\%$ $[59.1,64.8]$ & $53.5\%$ $[50.8,56.3]$ \\
Participant-label permutation $p$ & $0.0099$ & $0.0099$ \\
\hline
\end{tabular}
\end{table}

The reservoir's absolute accuracy declines as the task moves from three to four labels, as expected for a more granular decision. Its margin over the matched conventional features does not decline. This persistence is the defensible cross-granularity finding. Raw ERP-16 remains stronger at both granularities, which rules out an accuracy-superiority claim for the reservoir.

\subsection{Archived Within-Valence Comparisons}

The archived pairwise analysis displays $69.0\%$ for Cute versus Erotic and $52.8\%$ for Threat versus Cute. Because stimulus-level arousal and valence ratings were not modeled and the archived features used the nonmatching preprocessing path, these values do not establish that the reservoir is more sensitive to arousal than valence. That hypothesis requires a corrected model with continuous stimulus ratings and participant-grouped evaluation.

\subsection{Four-Class Clinical Findings}

The archived four-category screen contains $11$ nominal PTSD channel--metric cells, $28$ nominal PTSD edges, an uncorrected PTSD-by-category interaction ($p=0.036$), and an uncorrected SUD-by-efficiency interaction ($p=0.008$). These tests arose within broad exploratory families, were not independently replicated, and were not rerun as corrected confirmatory analyses. They therefore do not establish threat specificity, disorder-dependent network reorganization, cross-granularity replication, or biomarkers. Their appropriate role is to define prespecified hypotheses for a new sample.
...
The archived emotion and clinical ablation matrices suggest that the measured feature families are not numerically interchangeable. The embedding has the highest displayed emotion value; clinical values are near chance and vary by label and feature family. Because preprocessing was global and no clinical readout supplies confirmatory discrimination, these matrices do not establish disorder-specific sensitivity or information accessible only through the multi-layer architecture. They motivate a corrected, prespecified ablation in an independent sample.
...
The corrected contribution is a fold-safe, participant-grouped evaluation of a fixed temporal spike code at two label granularities, accompanied by temporal-order, temporal-collapse, electrode-identity, population-count, seed, temporal-window, fusion, and participant-label-permutation analyses. LSM and SNN emotion classification predate this work; novelty lies in the specified SHAPE application, the inspectable intermediate code, and the controls that delimit what it uses. Archived propagation, nuisance, ablation, centering, and clinical analyses remain hypothesis-generating. The strongest SUD parametric comparison has $p=0.0004$, but its label-permutation result is $p=0.073$.
...
The archived clinical screens illustrate how neuromorphic embeddings can generate graph-level hypotheses, but they do not establish a first clinical characterization, a coherent SUD phenotype, or information missed by spectral features. The nominal strength, efficiency, and clustering contrasts arose within broad search families; the strongest comparison does not pass the participant-label permutation test. Their defensible impact is to supply explicit, prespecifiable targets for independent replication.

More broadly, the archived increase from $63.4\%$ to $79.4\%$ after complete-panel centering motivates explicit study of participant nuisance. It does not demonstrate a prospective gain or show that condition signal is generally stronger than published EEG accuracies suggest, because the centering operation is transductive and preprocessing was global.


\section{Scope and Future Directions}
\label{sec:ch5_scope}

The present study opens several directions for further investigation. Electrode labeling, once obtained from the collaborating laboratory, will enable cortical mapping of the channel-level findings to standard brain regions. The graph operator analysis can be extended to learnable heterophily-aware architectures (e.g., FAGCN, GPR-GNN) and graph transformers with positional encodings, which may discover relational structure invisible to the fixed operators tested here. The SUD interaction effects require independent replication, family-wise multiplicity control, and a prespecified confirmatory test; the present permutation check yielded $p = 0.073$. The nuisance-structure discovery, that subject and condition share principal components, motivates the development of adversarial subject-invariant representation learning as a principled alternative to per-subject centering for deployment settings where subject-centering is not possible. The cross-sectional clinical metadata motivates longitudinal extensions to disentangle trait, state, and comorbidity contributions to the observed network phenotypes.
...
\end{tabular}
\end{center}
In this archived sweep, finer temporal resolution does not improve the displayed classification value. That pattern is qualitatively compatible with an information-bottleneck account \cite{tishby_deep_2015}, but mutual information was not estimated and the experiment cannot locate a compression--relevance frontier. Increased dimensionality, regularization, or pipeline-specific variance are alternative explanations.
...
The condition-averaged attention profiles are more differentiated after reservoir transformation than on raw EEG bins: Negative stimuli peak at the final temporal bin ($\alpha = 0.177$), Neutral at the fifth bin ($\alpha = 0.176$), and Pleasant at the fourth bin ($\alpha = 0.180$). On raw EEG bins, the profiles are flatter. These are descriptive population-average patterns, not evidence of condition separation at the individual-observation level; the permutation test below yields $p = 0.634$.
...
\paragraph{Channel Attention Is Near-Uniform.} The channel attention entropy ($3.524$ nats) differs from uniform ($3.526$ nats) by only $0.1\%$, indicating no meaningful spatial selectivity at this sample size ($N = 211$). Two design factors may contribute. First, per-subject centering removes subject-specific spatial topography that dominates the uncentered embedding and may also remove structure available to channel attention. Second, the SHAPE files use unlabeled channel indices, preventing anatomical interpretation of spatial weights. The present experiment therefore does not validate the channel-attention module as a spatial explanation mechanism. NeuCube \cite{kasabov_neucube_2014}, by comparison, imposes brain-atlas structure through Talairach coordinate mapping.

\paragraph{Prototype Assignments Provide Case-Based Summaries.} Ten of fifteen prototypes achieve classification purity above $60\%$, with the most selective prototype (Neutral class) reaching $93\%$ purity. Negative$\leftrightarrow$Pleasant cross-assignment is substantially more common than either condition's confusion with Neutral, consistent with an arousal-based interpretation. Prototype purity is a cohort-level descriptive quantity; its calibration for individual clinical decisions is not evaluated here.
...
\paragraph{Interpretability operates at two complementary levels.} The pre-reservoir temporal bin representation preserves ERP-aligned structure ($r = 0.65$ with P300, $|r| = 0.82$ with LPP) and supports condition-discriminative classification ($67.0\%$ balanced accuracy after centering). The post-reservoir BSC$_6$ spike representation has weaker ERP-amplitude correspondence ($r = 0.33$ with P300, $|r| = 0.23$ with LPP) and produces descriptively differentiated condition-average attention profiles. Because those profiles do not survive permutation testing, the supported claim is temporal traceability, not condition-specific clinical explanation.
...
\paragraph{Prototype purity provides a candidate confidence measure.} The prototype purity distribution ($53\%$--$93\%$) and condition-specific confusion structure provide cohort-level measures that could support case-based confidence estimation. Calibration, prospective reliability, and clinician use are not tested in this dissertation.

\paragraph{Significance for the field.} The chapter combines variance decomposition ($\rho = 7.2$), centering gains ($+13$ to $+21$~pp across seven representations), shared-subspace geometry (principal angles $= 0^\circ$), an attention-weighted readout, and prototype classification within one neuromorphic EEG pipeline. The attention results provide descriptive temporal localization but not inferential evidence of condition-specific separation. The prototype-based readout, inspired by ProtoPMed-EEG \cite{barnett_protopmed_2024}, extends case-based representation to a characterized reservoir transformation. Within the comparison developed in Chapter~\ref{chap:background}, these results support concurrent geometric transparency and temporal traceability without claiming clinical calibration.
...
The preceding sections show that ARSPI-Net's attention readout produces temporally indexed, condition-averaged profiles, although those profiles are not statistically separable by condition. This subsection compares the learned attention weights with post-hoc Input$\times$Gradient saliency computed on EEGNet, the highest-accuracy model in the comparison ($86.3\% \pm 4.0\%$ centered balanced accuracy in this experiment\footnote{The baseline table (Section~\ref{tab:baselines}) reports $89.1\%$ for EEGNet using the canonical training protocol with ReduceLROnPlateau scheduling and $200$ epochs. The saliency experiment uses a simpler training loop ($150$ epochs, fixed learning-rate schedule) for compatibility with gradient computation. The $2.8$~pp difference reflects training-protocol variation.}, using $10$-fold StratifiedGroupKFold).
...
\paragraph{EEGNet's unbinned saliency peaks later than the BSC$_6$ attention span.} The temporal saliency maxima occur at $691$~ms for Negative, $441$~ms for Neutral, and $402$~ms for Pleasant. The Negative maximum is later than a conventional LPP window, whereas the Neutral and Pleasant maxima fall near the LPP onset range. All three occur later than the $39$--$273$~ms span represented by the six ARSPI-Net attention bins. The comparison therefore identifies a difference in temporal weighting, not a failure of EEGNet to use physiologically plausible activity.

\paragraph{ARSPI-Net attention peaks within predefined early post-stimulus bins.} ARSPI-Net's condition-average BSC$_6$ attention peaks at $254$~ms for Negative, $214$~ms for Neutral, and $176$~ms for Pleasant. These peaks demonstrate direct temporal indexing of the learned readout. Because the condition-profile permutation test yields $p = 0.634$, they do not establish distinct temporal strategies or identify a physiological ERP component as the cause of an individual prediction.
...
\caption{Temporal attribution comparison between EEGNet (Input$\times$Gradient saliency, top row) and ARSPI-Net (learned attention weights, bottom left/center). EEGNet's unbinned saliency maxima occur at $691$~ms (Negative), $441$~ms (Neutral), and $402$~ms (Pleasant). The bottom-right panel compares EEGNet saliency and ARSPI-Net attention after binning to the same six early post-stimulus intervals. The ARSPI-Net condition-profile permutation test is not significant ($p = 0.634$).}
\label{fig:shap_comparison}
\end{figure}

\paragraph{Interpretation.} Both quantities are model-dependent attribution summaries. Input$\times$Gradient is post hoc, while ARSPI-Net attention is learned within a pipeline whose reservoir transformation and temporal bins are explicitly defined. The fixed reservoir improves traceability from an attention weight to a known temporal bin, but it does not guarantee that attention is a faithful causal explanation or prevent the learned readout from exploiting nuisance structure. The measured accuracy difference therefore cannot be interpreted as the cost of interpretability; it compares different model classes and training procedures. The supported result is narrower: the two methods emphasize different temporal regions, and ARSPI-Net exposes its weighting on predefined bins.
...
This chapter separates four corrected findings from four hypothesis-generating archived analyses. The corrected samples contain $211$ participants ($633$ observations) for three conditions and $210$ participants ($840$ observations) for four categories.

First, corrected participant-grouped evaluation places BSC$_6$/SRP-64 at $60.6\%$ for three conditions and $53.2\%$ for four categories. The corresponding advantages over conventional band-power/Hjorth features are $11.3$ and $16.5$ percentage points, with participant-label permutation $p=0.0099$ for both tasks.

Second, temporal-bin shuffle, temporal collapse, channel shuffle, and population-count controls reduce accuracy. The intact representation therefore uses temporal order and electrode identity, not only global spike totals.

Third, three fixed reservoir seeds give closely similar results, and temporal-window ablations show useful information across early, middle, and late windows. The full corrected window reaches $63.3\%$ in the three-repeat robustness experiment.

Fourth, raw ERP-16 outperforms the reservoir at both granularities ($68.1\%$ and $60.5\%$), setting an important boundary: ARSPI-Net is not an accuracy leader on this dataset.

Fifth, the archived graph-propagation sweep displays no improvement over the non-propagated representation. Global preprocessing prevents that sweep from establishing a universal graph operating boundary.

Sixth, archived variance decomposition and complete-panel centering motivate research on subject nuisance, but the centering estimand is transductive and not prospective.

Seventh, the archived layer ablation suggests that embedding, dynamics, topology, and coupling summarize different aspects of the signal. Near-chance clinical readouts and global preprocessing preclude a claim of disorder detection or superiority to single-stage classifiers.

Eighth, archived clinical screens yield nominal candidates but no validated biomarkers. The scientific value of retaining them is transparency and the generation of prespecified replication targets.
