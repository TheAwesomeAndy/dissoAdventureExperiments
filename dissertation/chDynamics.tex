...
The preceding chapters established ARSPI-Net as a hybrid architecture in which a Liquid State Machine performs temporal state expansion and a graph-based back end organizes channels. This chapter asks whether the reservoir trajectory can be summarized by named quantities with defined units or scales, and whether those quantities show stable correspondence with the input and affective condition.

The analysis uses the selected reservoir operating point ($\beta = 0.05$, $M_{th}=0.5$, $N_{res}=256$), extracts seven state-space descriptors from all $633$ SHAPE observations, and tests variation by affective condition and clinical label. The three metric families summarize sparse activity, driven-trajectory complexity, and relaxation behavior. They make the reservoir response analyzable without implying that the summaries are clinically actionable.

All seven dynamical metrics vary by condition (Friedman $p < 0.05$), with the temporal-structure family ($\hat{H}_{\pi}$, $\tau_{AC}$) showing $2.4\times$ larger effects than the amplitude-tracking family. None of the $49$ channel-averaged metric--diagnosis comparisons reaches significance. The layer ablation gives near-chance clinical balanced accuracies even where dynamical features are the best-performing ARSPI-Net layer. The excitability--persistence axis is therefore a cohort-level summary of reservoir organization, not a patient-level clinical scale.
...
\begin{proposition}[Range and equality cases of normalized permutation entropy]
\label{prop:permutation_entropy_bounds}
For $d\ge2$, $0\le\hat H_\pi\le1$. Equality $\hat H_\pi=0$ occurs when one
ordinal pattern has probability one; equality $\hat H_\pi=1$ occurs when all
$d!$ patterns have probability $1/d!$.
\end{proposition}

\begin{proof}
Shannon entropy on a finite alphabet is nonnegative and is maximized by the
uniform distribution. The ordinal alphabet has at most $d!$ symbols, so
$0\le H_\pi\le\log_2(d!)$. Division by $\log_2(d!)$ gives the result. The
standard equality conditions for finite-alphabet entropy give the two cases.
\end{proof}

The proposition supplies an exact scale interpretation, but finite-sample bias,
ties, and temporal dependence still affect estimation of the underlying pattern
probabilities.
...
\begin{enumerate}
    \item a measurement-instrument framing that clarifies what the reservoir is and is not being claimed to measure;
...
\subsection{Measurement-Instrument Framing}
...
In particular, the dissertation does not claim that the reservoir relaxation time is literally the relaxation time of the cortex. Rather, the reservoir relaxation time, when extracted under conditions supporting input determinism and adequate fidelity, is treated as a functional of the EEG drive. This measurement-instrument framing treats the reservoir as a standardized nonlinear transform rather than as a direct physiological surrogate.
...
\end{enumerate}
These three questions provide the theoretical validation structure for this chapter.
...
\end{equation}
after transient decay, for the specific operating regime under study.

For the implemented update, one conditional result can be proved without a
smooth global approximation.

\begin{theorem}[Conditional contraction for a fixed spike itinerary]
\label{thm:conditional_lif_contraction}
Consider two reservoir trajectories driven by the same input. If their spike
indicators are identical through time $t$, then, with $\alpha=1-\beta$,
\begin{equation}
\|\mathbf{R}[t]-\mathbf{R}'[t]\|_\infty
\le\alpha^t\|\mathbf{R}[0]-\mathbf{R}'[0]\|_\infty.
\end{equation}
\end{theorem}

\begin{proof}
Identical spike indicators make the recurrent currents and threshold
subtractions identical. The difference update is therefore
$\Delta\mathbf{R}[k]=\alpha\operatorname{diag}(1-\mathbf{S}[k-1])
\Delta\mathbf{R}[k-1]$. The diagonal mask has induced infinity norm at most one.
Applying this inequality recursively proves the bound.
\end{proof}

The theorem is deliberately conditional. A threshold crossing can change the
itinerary, so it does not establish a global ESP. Together with the margin lemma
in Chapter~\ref{chap:lsm}, it does prove local exponential forgetting on every
finite region whose spike decisions remain unchanged.

\subsection{Fading Memory and Effective Memory Horizon}

Once the state is a function of the input history, one still needs a practical bound on how much history matters. Reservoir systems with the fading-memory property weight remote lags less strongly than recent lags. The relevant engineering consequence is that there exists an effective memory horizon $T_{mem}$ such that perturbations beyond that horizon have negligible effect on the present state. Relating this horizon to the driven contraction rate through $T_{mem}\approx -1/\lambda_1$ is a useful operating heuristic, not an exact identity. Under the fixed-itinerary assumptions of Theorem~\ref{thm:conditional_lif_contraction}, the exact discrete decay is instead $\alpha^k$, yielding the tolerance-dependent horizon in Proposition~\ref{prop:subthreshold_horizon}.
...
The same logic extends to Fisher-information language: any reservoir-derived metric is an estimator whose efficiency must be discussed relative to the information present in the observed EEG, not the unobserved cortical state.
...
The driven Lyapunov exponent is a joint property of the coupled system, not a direct estimator of the cortical Lyapunov spectrum. It depends on the instantaneous reservoir Jacobian along the driven trajectory and therefore on how the input positions the reservoir relative to threshold. The reservoir's driven stability may be modulated by input structure, but it is not the brain's Lyapunov exponent.

\paragraph{Relaxation-time metrics.}
Among the three families, relaxation-time metrics admit the strongest transfer argument. During the decay phase the reservoir can be approximated locally by a linearized driven system, so the observed decay constant of the population rate depends on both intrinsic reservoir contraction and the autocorrelation structure of the ongoing input. It is therefore an input-mediated effective time constant rather than a direct cortical time constant.
...
The seven experiments establish the dynamical response layer as the primary locus of ARSPI-Net's state-space characterization. Each descriptor is a named quantity with a defined unit or dimensionless scale and a direct mapping to the reservoir's driven response: total spikes (count), mean firing rate (spikes per neuron per timestep), rate entropy (bits), rate variance (spikes$^2$), temporal sparsity (fraction of timesteps with at least one active neuron), permutation entropy (normalized, dimensionless), and autocorrelation time (timesteps). These quantities can be compared with their cohort distributions and with the excitability--persistence axis (Section~\ref{sec:ch6_latent_axis}). Their advantage over opaque hidden units is inspectability; patient-level reliability and clinical actionability are not evaluated.
...
The seven dynamical descriptors developed in this chapter are condition-sensitive (Section~\ref{sec:ch6_results}) and organize subjects along the excitability--persistence axis (Section~\ref{sec:ch6_latent_axis}). This section evaluates a stronger property, the third level of the interpretability taxonomy (Section~\ref{sec:interp_taxonomy}): correspondence with classical ERP component measurements used in affective electrophysiology.
...
This result establishes the amplitude-domain component of Level~3 interpretability: the mean-amplitude descriptor recovers classical ERP scalars within the measured feature window. The matching analysis on the reservoir's BSC$_6$ spike output (Chapter~\ref{chap:arspi_net}, Section~\ref{sec:erp_recovery}) yields weaker alignment ($|r| = 0.23$ for LPP), indicating that the reservoir does not preserve amplitude correspondence to the same degree. Together, the analyses distinguish strong amplitude-domain traceability from weaker spike-domain correspondence without asserting clinical explanation fidelity.

Among existing SNN architectures, NeuCube \cite{kasabov_neucube_2014} provides STDP-based connectivity analysis; Doborjeh et al.\ \cite{doborjeh_explainable_2021} showed that its learned connectivity patterns distinguish clinical groups. ARSPI-Net's Level~3 contribution differs in two respects. First, its descriptors are properties of the \emph{driven} reservoir trajectory under a fixed transformation rather than of adapted connection weights, making them deterministic functions of the input for a fixed reservoir instantiation. Second, the per-channel descriptor-to-ERP alignment ($|r| = 0.82$ for LPP in the amplitude domain and $|r| = 0.23$ in the spike domain) provides a quantitative bridge between the reservoir vocabulary and conventional ERP measurements. ProtoPMed-EEG \cite{barnett_protopmed_2024} does not provide dynamical characterization, and EEGNet's hidden representations are not evaluated by the same descriptor-to-ERP test. The excitability--persistence axis (Section~\ref{sec:ch6_latent_axis}) integrates the seven descriptors into a one-dimensional cohort-level summary.
