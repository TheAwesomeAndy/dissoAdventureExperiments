...
> **Important scope note.** Passing verification tests establish code behavior, not scientific validity (see [`VERIFICATION_METHODOLOGY.md`](VERIFICATION_METHODOLOGY.md)). The tier of a result — confirmatory vs. archived — is a statement about its study design and preprocessing boundary, not about whether its test suite passes. All 453 tests pass regardless of tier.
...
All rows are produced by the committed confirmatory runner
`experiments/confirmatory/run_confirmatory_validation.py` (library:
`confirmatory_pipeline.py`). The runner writes `confirmatory_results.json`,
which `check_against_manifest.py` compares against `results_manifest.json`.

| Dissertation Element | Value | Produced by |
|---|---|---|
| Table (corrected validation): BSC₆/SRP-64 | 60.6% [57.8, 63.4] (3-cond); 53.2% [50.6, 55.8] (4-cat) | `run_confirmatory_validation.py` |
| Raw ERP-16 reference | 68.1% [65.1, 71.1] (3-cond); 60.5% [57.9, 63.0] (4-cat) | `run_confirmatory_validation.py` |
| Conventional spectral/Hjorth baseline | 49.4% (3-cond); 36.7% (4-cat) | `run_confirmatory_validation.py` |
| Fusion (BSC₆/SRP-64 + conventional) | 62.0% (3-cond); 53.5% (4-cat) | `run_confirmatory_validation.py` |
| Advantage over conventional | +11.3 pp [7.9, 14.7] (3-cond); +16.5 pp [13.3, 19.8] (4-cat) | `run_confirmatory_validation.py` |
| Participant-label permutation (ridge) | p = 0.0099 (both granularities) | `run_confirmatory_validation.py` |
| Destruction controls (temporal-bin shuffle / collapse / electrode shuffle) | −7.4/−4.9/−13.9 pp (3-cond); −12.9/−8.6/−20.1 pp (4-cat) | `run_confirmatory_validation.py` |
| Seed robustness (7, 42, 99) | 60.5–62.1%; full window 63.3% (3-cond) | `run_confirmatory_validation.py` |

> **Reproducibility contract for restricted data.** The runner reuses the
> validated LIF reservoir and BSC₆ encoder from
> `chapter5Experiments/experiment_zero.py`; it replaces the archived globally
> fitted PCA-64 with a fixed, data-independent `SparseRandomProjection`
> (SRP-64) and fits standardization fold-locally under
> `StratifiedGroupKFold`. Because the SHAPE dataset is restricted and not
> shipped here, the exact headline numbers are regenerated by feeding the
> authorized data to the runner. The *machinery* — SRP data-independence,
> fold-local fitting, participant grouping, determinism, and output schema —
> is verified without the dataset by
> `experiments/confirmatory/verify_confirmatory.py` (18 checks), so it runs in
> CI. This is the honest separation: verified code here, numbers from
> authorized data.
...
| Confirmatory pipeline (synthetic, no data) | `experiments/confirmatory/verify_confirmatory.py` | 18 |
| **Total** | | **453** |
