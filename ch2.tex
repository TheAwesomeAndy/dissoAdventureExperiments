...
The contemporary EEG literature makes evaluation design part of the scientific claim rather than a secondary implementation detail. Lee et al.\ compared subject-wise and trial-wise validation after cropping-based EEG augmentation and showed that placing segments from the same participant in both training and test sets inflates diagnostic estimates \cite{lee_eeg_leakage_2023}. The result is directly relevant whenever one participant contributes multiple epochs, conditions, or augmented windows: the split must occur at the participant level if the intended claim concerns unseen participants.
