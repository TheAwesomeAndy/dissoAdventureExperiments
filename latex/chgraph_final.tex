...
The experimental program produces four principal findings. First, variance decomposition reveals that subject identity accounts for $62.6\%$ of embedding variance, condition accounts for $8.7\%$, and residual within-subject variability accounts for $28.7\%$---a $7\times$ subject-to-condition dominance ratio. Removing the subject-identity component through per-subject centering increases classification accuracy from $63.4\%$ to $79.4\%$ balanced accuracy, demonstrating that the reservoir's condition-discriminative capacity is substantially richer than cross-subject classification accuracy suggests. Second, a Propagation Operating Characteristic analysis across six graph densities, six propagation depths, three adjacency families, and four contrast-preserving operator families demonstrates that \emph{no graph operator}---smoothing, high-pass, residual, difference-aware, or attention-based---exceeds the non-propagated baseline. The graph Laplacian spectral analysis reveals the mechanism: propagation acts as a graph-spectral low-pass filter that suppresses high-frequency inter-channel contrast (which carries condition signal) faster than it suppresses low-frequency subject-nuisance structure. Third, the same graph, treated as a \emph{descriptive topological object} rather than a propagative inference substrate, reveals distributed transdiagnostic network phenotypes, with the strongest signal in Substance Use Disorder across three complementary metrics. Fourth, PCA nuisance regression monotonically destroys accuracy because subject and condition share principal components---a structural discovery about the geometry of neuromorphic EEG embeddings.

The chapter's central contribution is a regime characterization, not a successful graph classifier. The novel scientific object is a \emph{detection-theoretic understanding} of when and why graph operators increase or decrease the detectability of a weak task signal embedded in strong nuisance structure on finite-sensor arrays. This understanding, formalized through the Propagation Operating Characteristic and the graph-spectral condition signal analysis, provides the EEG-GNN field with a principled framework for graph operator selection.
...
\begin{figure}[H]
    ...
    \caption{Nuisance-suppression strategy comparison ($6$ methods). Left: subject-centering ($79.4\%$) dominates all alternatives. Center: PCA nuisance regression monotonically destroys accuracy because subject and condition share principal components. Right: subject-centering + PCA reduction---full $2{,}176$-d centered representation is optimal.}
...
Subject-centering followed by PCA compression to $100$ dimensions achieves $72.2\%$---below the full $2{,}176$-d centered result ($79.4\%$), indicating that the condition signal is distributed across many dimensions and benefits from the full representational capacity of the reservoir embedding.
...
This chapter makes five novel contributions. The first is a \textbf{signal-detection analysis}: variance decomposition and subject-nuisance suppression reveal that the reservoir's condition-discriminative capacity is $79.4\%$ balanced accuracy ($+16.0$~pp over the subject-contaminated estimate), with the improvement confirmed across six independent detection metrics (d$'$, Bhattacharyya, FDR, Mahalanobis, MI lower bound). The second is a \textbf{regime characterization}: the Propagation Operating Characteristic, swept across six densities, six depths, three adjacency families, and four contrast-preserving operator families, demonstrates that no graph operator tested---smoothing, high-pass, residual, difference-aware, or attention-based---exceeds the non-propagated baseline on this graph class. The third is a \textbf{nuisance-structure discovery}: PCA nuisance regression monotonically destroys accuracy (to below chance at $50$ PCs removed) because subject and condition share principal components---subject-centering succeeds by removing the per-subject mean without altering the variance structure. The fourth is an \textbf{architectural finding}: the performance gap between propagation and non-propagation \emph{widens} with increasing sample size ($N = 115$ to $N = 211$), confirming a structural rather than statistical boundary. The fifth is a \textbf{clinical discovery}: SUD modulates emotional network reactivity across three complementary graph metrics, validated by permutation testing ($2.3\sigma$ above null) and internal replication.

\subsection{Significance for Electrical Engineering}

The Propagation Operating Characteristic provides the field with a systematic framework for evaluating graph operators on finite sensor arrays. The heterophily-aware operator experiment extends this characterization beyond message-passing: six operator families (GCN, high-pass, residual, difference-aware, self-attention, and no-propagation) were tested, and the non-propagated representation maximizes detection-theoretic separability across all six metrics tested (d$'$, Bhattacharyya, FDR, Mahalanobis, MI lower bound, and classification accuracy). The nuisance-structure analysis reveals that subject and condition share principal components---a finding with implications for any EEG study that uses PCA preprocessing. Subject-centered reservoir embeddings achieve $79.4\%$ balanced accuracy on three-way emotion classification from a transdiagnostic clinical sample ($N = 211$, mean comorbidity $2.8$), establishing a new benchmark for trial-averaged ERP classification. The regime characterization generalizes to any small dense sensor graph (ECG, EMG, fNIRS), providing a design criterion: for sensor graphs with $< 50$ nodes and $> 15\%$ density, per-channel feature extraction with nuisance suppression preserves more discriminative information than any graph propagation strategy tested.
...
This chapter makes five contributions validated on $211$ subjects ($633$ observations).

First, variance decomposition establishes that subject identity accounts for $62.6\%$ of embedding variance, with condition signal at $8.7\%$ ($7\times$ ratio). Subject-centering increases classification accuracy from $63.4\%$ to $79.4\%$ ($+16.0$~pp), confirmed by six independent detection metrics. PCA nuisance regression monotonically destroys accuracy because subject and condition share principal components.

Second, a Propagation Operating Characteristic across six graph densities ($5$--$50\%$), six propagation depths ($K = 0$--$5$), and three adjacency constructions demonstrates that message-passing monotonically reduces accuracy at every operating point, absorbing up to $90\%$ of inter-channel discriminative variance at high density.

Third, four contrast-preserving graph operators (high-pass, residual skip-connection, signed-difference aggregation, channel self-attention) recover to the non-propagated baseline but do not exceed it. Across all operator families tested, the non-propagated per-channel representation maximizes classification accuracy on this graph class.

Fourth, a comprehensive detection-theoretic analysis across six metrics and six representations shows that the reservoir achieves $10\times$ separability improvement over band-power features, subject-centering adds $2.8\times$, and no graph operator improves detectability beyond the per-channel representation.

Fifth, graph-topological analysis---using the graph descriptively rather than propagatively---reveals that Substance Use Disorder modulates emotional network reactivity across three complementary metrics ($p = 0.0004$, $0.003$, $0.012$), validated by permutation testing ($2.3\sigma$ above null) and internal replication at two sample sizes.
