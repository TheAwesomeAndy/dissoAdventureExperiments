...
To our knowledge, ARSPI-Net is the first architecture that achieves all four levels through a single staged pipeline: temporal bin activations that correlate with P300 ($r = 0.65$) and LPP ($|r| = 0.82$) amplitudes (Level~1, Chapter~\ref{chap:embeddings}), variance decomposition revealing the $7.2\times$ subject-to-condition ratio with universal centering gains across seven representations (Level~2, Chapter~\ref{chap:arspi_net}), six named descriptors that collectively predict $66\%$ of LPP variance (Level~3, Chapter~\ref{chap:dynamics}), and structure--function coupling with prototype-based clinical profiling (Level~4, Chapter~\ref{chap:synthesis}). This four-level interpretability is intrinsic---it requires no post-hoc attribution methods because the pipeline's intermediate variables are themselves clinically meaningful quantities.
...
\midrule
L1: Temporal traceability ($r = 0.82$ LPP)   & \checkmark & \texttimes & \checkmark & $\sim$ \\
L2: Geometric transparency ($\rho = 7.2$)    & \checkmark & \checkmark & \texttimes & $\sim$ \\
L3: Dynamical characterization ($R^2 = 0.66$) & \checkmark & \texttimes & \checkmark & \texttimes \\
