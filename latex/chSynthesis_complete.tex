...
The preceding chapters established two distinct characterization axes for affective EEG processing in ARSPI-Net. Chapter~\ref{chap:dynamics} developed a dynamical characterization framework that extracts per-electrode temporal descriptors---validated through a five-gate chain of driven stability, cross-initialization reliability, transformation specificity, surrogate sensitivity, and affective deployment---from the reservoir's internal state evolution. Chapter~5 developed the graph-theoretic axis by treating electrodes as nodes in a functional connectivity graph and characterizing propagation regimes, nuisance-suppression properties, and topological clinical phenotypes. Each axis carries information about the underlying EEG: the temporal axis measures how the reservoir transforms each channel's signal, while the spatial axis measures how channels are organized relative to one another.

The present chapter asks whether these two axes are aligned. In the fMRI structure--function coupling literature, alignment between local functional properties and global topological role is a hallmark of organized neural processing \cite{Deco2015,honey_network_2007,suarez_linking_2020}. If an electrode that occupies a central position in the functional graph also produces a distinctive reservoir trajectory, then the temporal and spatial characterizations are coupled views of a single underlying signal organization rather than independent descriptions. If they are not aligned, the two axes access genuinely different layers of the EEG. Both outcomes are informative: coupling reveals systems-level coordination, while independence reveals that the two-axis instrument captures non-redundant structure.

The epistemic discipline established in Chapter~\ref{chap:dynamics} extends to this chapter. Coupling metrics are properties of two instrument-derived descriptor families---the driven reservoir response and the connectivity-derived graph topology---not direct measurements of cortical structure--function coupling. The chapter tests whether these instrument outputs are aligned across electrodes, whether that alignment varies with affective content, and whether it carries clinical information. It does not claim to measure latent cortical organization.

Five experiments are organized in a deductive chain. Experiment~7.1 tests whether coupling exists above permutation-null baselines. Experiment~7.2 decomposes the variance of coupling strength into subject and category components, directly paralleling Chapter~5's variance decomposition methodology. Experiment~7.3 examines whether the structure of coupling---which specific dynamical-topological relationships shift---differs across within-valence affective subcategories. Experiment~7.4 tests whether diagnosis-associated groups differ in coupling strength. Experiment~7.5 asks whether the two descriptor families carry complementary discriminative information for clinical detection, or whether their value is organizational rather than discriminatively additive.
...
Five experiments converted the relationship between ARSPI-Net's temporal and spatial characterization axes from a hypothesis into a quantified empirical result.

Experiment~7.1 established that electrode-wise coupling between the dynamical and topological descriptor families exists above the electrode-permutation null with $d_z = 1.063$ ($p = 5.0 \times 10^{-113}$). The group-mean coupling matrix is uniformly negative: electrodes with higher connectivity produce simpler reservoir trajectories, consistent with the signal-processing interpretation that phase-locked inputs deliver lower effective diversity to the reservoir. The temporal-structure metrics identified in Chapter~\ref{chap:dynamics}---permutation entropy and $\tau_{AC}$---are the metrics most strongly aligned with topology.

Experiment~7.2 decomposed coupling variance into subject (29.2\%), category (0.6\%), and residual (70.1\%) components. The subject fraction is less than half the 62.6\% observed in Chapter~5's embedding variance, revealing that coupling accesses a different layer of signal organization than the raw descriptors. The ICC of 0.059 demonstrates that coupling is observation-specific rather than trait-like---each stimulus--subject combination produces a largely independent coupling configuration.

Experiment~7.3 opened the coupling matrix and identified $\tau_{AC}$ as the primary carrier of the Cute--Erotic coupling reorganization, with opposite-direction shifts for the Threat--Mutilation contrast. Individual cell effects do not survive Bonferroni correction but are distributed as coordinated low-amplitude shifts across 12 of 14 cells, establishing that affective modulation of coupling is a pattern-level phenomenon.

Experiment~7.4 established that diagnosis-associated groups do not differ in coupling strength at the scalar $\bar{\kappa}$ level (all $|d| < 0.12$). The temporal and spatial axes each carry diagnosis-associated signals independently (Chapters~5 and~\ref{chap:dynamics}), but their alignment does not carry the same signal. The coupling layer is a different dimension of EEG organization.

Experiment~7.5 established that combining topological and dynamical descriptors never improves classification beyond the better individual family. The descriptors are organizationally related but not discriminatively complementary. ADHD is the one diagnosis where the reservoir's temporal descriptors carry a unique signal (AUC $= 0.622$) and where the scalar $\kappa$ provides non-redundant augmentation information ($p = 0.001$).
